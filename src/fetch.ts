...
function dedup(results: FetchResult[]): {
  ...
  for (const { source, config } of results) {
    if (source.category === 'acl4ssr' && !templateAcl4ssr) {
      templateAcl4ssr = config;
    }
    if (source.category === 'freesub' && !templateFreesub) {
      templateFreesub = config;
    }
    ...
    for (const proxy of config.proxies) {
      ...
      if (count > 0) {
        proxy.name = `${baseName}_${count}`;
      }
      nameCounter.set(baseName, count + 1);

      all.push(proxy);
      if (source.category === 'acl4ssr') acl4ssr.push(proxy);
      else if (source.category === 'freesub') freesub.push(proxy);
    }
  }
...
function buildFullConfig(template: ParsedConfig, proxies: Proxy[]): ParsedConfig {
...
async function main() {
  ...
  // save intermediate data for subs-check
  fs.mkdirSync(DATA_DIR, { recursive: true });

  // all nodes for subs-check input
  writeYaml(path.join(DATA_DIR, 'all-raw.yaml'), buildNodesOnly(all));

  // save category mapping (proxy name → category)
  const categoryMap: Record<string, string> = {};
  for (const p of acl4ssr) categoryMap[p.name] = 'acl4ssr';
  for (const p of freesub) categoryMap[p.name] = 'freesub';
  fs.writeFileSync(path.join(DATA_DIR, 'category-map.json'), JSON.stringify(categoryMap), 'utf-8');
